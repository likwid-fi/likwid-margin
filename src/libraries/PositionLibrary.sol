...
library PositionLibrary {
    ...
    function calculatePositionKey(address owner, bool isOne, bytes32 salt)
        internal
        pure
        returns (bytes32 positionKey)
    {
        assembly ("memory-safe") {
            ...
            // abi.encodePacked(owner, isOne, salt) is 53 bytes:
            // | owner (20 bytes) | isOne (1 byte) | salt (32 bytes) |

            // We construct the first 32 bytes of the packed data:
            // | owner (20 bytes) | isOne (1 byte) | salt (first 11 bytes) |
            // Shift owner left by 12 bytes (96 bits) to align it to the start of the word.
            let word1 := shl(96, owner)
            // Shift isOne left by 11 bytes (88 bits) to place it right after the owner.
            word1 := or(word1, shl(88, isOne))
            // Take the top 11 bytes (88 bits) of the salt and place them after isOne.
